...
//// Argument parsing
int g_print_help(const char * /*value*/) {
    char options[] = "\nOptions:\n"
                     "  -h,   --help\t\t\tprint help\n"
    ...
    std::cout << "usage: " << g_program_name << "[OPTIONS...]\n" << options << std::endl;

    return -1;
}

int arg_print_version(const char * /*value*/) {
    std::cout << PROGRAM_NAME << " " << VERSION << std::endl;
    return -1;
}

int arg_verbose(const char * /*value*/) {
    g_log_level = LOG_DEBUG;
    return 0;
}

int arg_print_frames(const char * /*value*/) {
...
Arg arguments[] = {
    {.long_form = "--help", .short_form = "-h", .type = ArgType::FLAG, .handler = &g_print_help},
...
int main(const int argc, const char **argv) {
    ...
    const int result = parse_arguments(argc, argv);
    if (result == 1) {
        return 1;
    } else if (result == -1) {
        // Early exit
